...
export default function PostCard({ post, isLink = true, onVote, onDelete, userVote, isPurifying = false, onPurificationComplete }: PostCardProps) {
    ...
    const handleCommentClick = (e: React.MouseEvent) => { e.stopPropagation(); e.preventDefault(); router.push(`/compose?parentId=${post.id}`); };
        ...
        >
            ...
            <div className="flex items-center justify-between text-sm text-gray-400 mt-3">
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                    ...
                    <button onClick={handleCommentClick} className={`icon-base icon-comment ${hasComments ? 'has-comments' : ''}`}><Icon name="comment" value={post.stats?.replyCount ?? 0} /></button>
                </div>
            </div>
    ...
    return (<Link href={`/post/${post.id}`} className="no-underline" scroll={false}> {cardContent} </Link>);
}